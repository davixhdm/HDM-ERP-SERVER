...
const buildContextData = async (tenantId, moduleScopes, question = '') => {
  ...
  if (moduleScopes.includes('crm') && shouldInclude(['lead', 'pipeline', 'crm', 'deal', 'opportunity'])) {
    ...
    Object.assign(data.summary, { totalLeads: leads.length, pipelineValue: leads.reduce((s, l) => s + (l.value || 0), 0), wonLeads: stageCounts.won, lostLeads: stageCounts.lost, conversionRate: leads.length > 0 ? Math.round((stageCounts.won / leads.length) * 100) : 0, leadStages: stageCounts });
  }
  ...
  if (moduleScopes.includes('projects') && shouldInclude(['project', 'task', 'deadline', 'milestone'])) {
    ...
    const [projects, tasks] = await Promise.all([Project.find({ tenantId }).sort({ createdAt: -1 }).limit(10).lean(), Task.find({ tenantId }).sort({ createdAt: -1 }).limit(50).lean()]);
    data.projects = projects.map(p => ({ name: p.name, status: p.status, priority: p.priority, budget: p.budget }));
    const doneTasks = tasks.filter(t => t.status === 'done').length;
    Object.assign(data.summary, { totalProjects: projects.length, activeProjects: projects.filter(p => p.status === 'active').length, totalTasks: tasks.length, completedTasks: doneTasks, pendingTasks: tasks.filter(t => t.status !== 'done').length, taskCompletionRate: tasks.length > 0 ? Math.round((doneTasks / tasks.length) * 100) : 0 });
  }
  ...
  if (moduleScopes.includes('assets') && shouldInclude(['asset', 'equipment', 'maintenance', 'depreciation'])) {
    ...
    const [assets, maintenances] = await Promise.all([Asset.find({ tenantId }).sort({ createdAt: -1 }).limit(20).lean(), Maintenance.find({ tenantId, status: { $in: ['scheduled', 'in_progress'] } }).lean()]);
    ...
    Object.assign(data.summary, { totalAssets: assets.length, totalAssetValue: assets.reduce((s, a) => s + (a.currentValue || 0), 0), totalPurchaseCost: assets.reduce((s, a) => s + (a.purchaseCost || 0), 0), assetsInMaintenance: assets.filter(a => a.status === 'maintenance').length, retiredAssets: assets.filter(a => a.status === 'retired').length, pendingMaintenance: maintenances.length, assetCategories: categoryCounts });
  }
...
const landingQuery = async (question, landingConfig) => {
  ...
  const plansData = landingConfig?.plansTextSummary || '';
  const enhancedMessage = plansData
    ? `PLAN DATA (use this exact data for answering pricing/plan questions):\n${plansData}\n\nUSER QUESTION: ${question}`
    : question;
  ...
  const payload = {
    ...
    context: {
      ...
      features: landingConfig?.allFeatures?.join(', ') || '',
      pricing: landingConfig?.pricingSummary || '',
      plansTextSummary: plansData,
      plans: landingConfig?.plans || [],
  ...
  try {
    ...
    logger.error(`Landing AI call failed: ${err.message}`);
    throw err;
  }
};
...
const runProactiveAlerts = async () => {
  try {
    const aiConfig = await AIConfig.findOne();
    if (!aiConfig || !aiConfig.features?.proactiveAlerts) {
      logger.info('Proactive alerts: Disabled');
      return;
    }
    ...
    for (const tenant of activeTenants) {
      try {
        ...
        if (data.summary?.lowStockItems > 0) alerts.push({ icon: '📦', title: 'Low Stock Alert', detail: `${data.summary.lowStockItems} item(s) below reorder level: ${data.summary.lowStockList?.slice(0, 5).join(', ')}`, severity: 'warning' });
        if (data.summary?.unpaidInvoices > 0) alerts.push({ icon: '📋', title: 'Unpaid Invoices', detail: `${data.summary.unpaidInvoices} unpaid invoice(s) totaling ${data.summary.currency} ${(data.summary.pendingRevenue || 0).toLocaleString()}`, severity: 'info' });
        if (data.summary?.unpaidBills > 0) alerts.push({ icon: '💰', title: 'Unpaid Bills', detail: `${data.summary.unpaidBills} unpaid bill(s) totaling ${data.summary.currency} ${(data.summary.pendingExpenses || 0).toLocaleString()}`, severity: 'info' });
        if (data.summary?.netProfit < 0) alerts.push({ icon: '📉', title: 'Negative Net Profit', detail: `Expenses exceed revenue by ${data.summary.currency} ${Math.abs(data.summary.netProfit).toLocaleString()}`, severity: 'danger' });
        if (data.summary?.pendingOrders > 0) alerts.push({ icon: '📝', title: 'Pending Sales Orders', detail: `${data.summary.pendingOrders} order(s) awaiting fulfillment`, severity: 'info' });
        if (data.summary?.pendingMaintenance > 0) alerts.push({ icon: '🔧', title: 'Pending Maintenance', detail: `${data.summary.pendingMaintenance} maintenance(s) need attention`, severity: 'warning' });
        if (data.summary?.pendingTasks > 0) alerts.push({ icon: '📐', title: 'Pending Tasks', detail: `${data.summary.pendingTasks} task(s) still in progress`, severity: 'info' });
        if (data.summary?.totalLeads > 5 && data.summary?.conversionRate < 20) alerts.push({ icon: '🎯', title: 'Low Lead Conversion', detail: `Only ${data.summary.conversionRate}% of ${data.summary.totalLeads} leads converted`, severity: 'warning' });

        if (alerts.length > 0 && tenant.contactEmail) {
          const alertCards = alerts.map(a => `<div style="margin:10px 0;padding:12px;border-left:4px solid ${a.severity === 'danger' ? '#EF4444' : a.severity === 'warning' ? '#F59E0B' : '#3B82F6'};background:#f9fafb;border-radius:4px;"><p style="margin:0;font-size:14px;"><strong>${a.icon} ${a.title}</strong></p><p style="margin:4px 0 0 0;font-size:13px;color:#4b5563;">${a.detail}</p></div>`).join('');
          await sendEmail({
            to: tenant.contactEmail, toName: tenant.companyName,
            subject: `🔔 HDM ERP — ${alerts.length} Alert(s) for ${tenant.companyName}`,
            htmlContent: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;padding:20px;border:1px solid #e5e7eb;border-radius:8px;"><div style="background:#10B981;padding:16px;text-align:center;border-radius:8px 8px 0 0;margin:-20px -20px 20px -20px;"><h2 style="color:#fff;margin:0;font-size:18px;">Proactive Business Alerts</h2></div><p style="font-size:14px;">Hello <strong>${tenant.companyName}</strong>,</p><p style="font-size:13px;color:#6b7280;">Here's what needs your attention:</p>${alertCards}<div style="text-align:center;margin-top:20px;"><a href="https://hdmerp.pxxl.click/login" style="display:inline-block;background:#10B981;color:#fff;padding:10px 24px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;">View Dashboard</a></div><p style="color:#9ca3af;font-size:11px;margin-top:20px;text-align:center;">You're receiving this because proactive alerts are enabled.</p></div>`
          });
          await AIUsageLog.create({ tenantId: tenant._id, query: '__proactive_alert__', tokensUsed: 0, provider: 'system', timestamp: new Date() });
          logger.info(`Proactive alert sent to ${tenant.companyName}: ${alerts.length} issue(s)`);
          alerted++;
        }
      } catch (tenantErr) { logger.warn(`Proactive alert skipped for ${tenant._id}: ${tenantErr.message}`); skipped++; }
    }
    logger.info(`Proactive alerts: ${alerted} sent, ${skipped} skipped, ${activeTenants.length} checked`);
  } catch (err) { logger.error('Proactive alerts failed:', err.message); }
};
